<template>
  ...
  >
    <label
      v-if="title"
      class="text-lg !font-bold !font-josefin text-violet-950 dark:text-violet-300 md:!text-xl w-full"
      >{{ title }}</label
    >
    <input
      ...
      :type="currentType"
      v-model="inputModel"
      class="focus:outline-none !text-lg md:py-2 px-4 !w-full h-14 md:!text-base hover:shadow-[0_0_16px_rgba(109,40,217,0.4)] focus:shadow-[0_0_16px_rgba(109,40,217,0.4)] transition-[background-color,outline,color,box-shadow] duration-200 ease-in-out text-zinc-950 dark:text-zinc-200"
      :class="divClass"
    />
  </fieldset>
</template>
